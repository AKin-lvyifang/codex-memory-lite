...
const zipHero = path.join(releaseFolder, "docs", "images", `codex-memory-lite-v${version}.png`);
fs.mkdirSync(path.dirname(zipHero), { recursive: true });
fs.copyFileSync(path.join(root, "docs", "images", `codex-memory-lite-v${version}.png`), zipHero);
